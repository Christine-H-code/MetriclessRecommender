...
class nonMetricBasedRecommender_v1():
    def __init__(self, data, idColumnName: str, itemColumnName: str,kFolds: int = 2, gradientStepSize: float = 0.01, categiricalColumns: list = None, numericalColumns: list = None, backDatingDate: dict = {"dateColumnName":None, "cutOffDate":None}, backDatedData = None,fold_size: int = 10000):
        ...
        itemColumns,self.nCategoricalColumns,self.aggUpToDateData = dataPreparation.data_preparation(data.copy(),idColumnName,itemColumnName)
        ...
        if self.backDatedData is None:
            ...
            self.aggBackDatedData = dataPreparation.data_preparation(self.backDatedData.copy(),idColumnName,itemColumnName)[2]
            ...
            self.recommendationData = recommendationScore.RecommendationScores(nRecommendations,self.optimalCosineWeightings, self.optimalJaccardWeightings,itemColumns,self.nCategoricalColumns,self.aggUpToDateData,itemColumnName )
...
class nonMetricBasedRecommender_v2():
    def __init__(self, data, idColumnName: str, itemColumnName: str,kFolds: int = 2, gradientStepSize: float = 0.01, categiricalColumns: list = None, numericalColumns: list = None, fold_size: int = 10000,tetsingMethod: str = "remove-one",evaluationMethod: str = "MRR4", backDatedData = None, backDatingDate: dict = {"dateColumnName":None, "cutOffDate":None}):
        ...
        self.itemColumnsTest,self.nCategoricalColumnsTest,self.aggTestData = dataPreparation.data_preparation(data.copy(),idColumnName,itemColumnName)
        ...
        self.itemColumns,self.nCategoricalColumns,self.aggTrainingData = dataPreparation.data_preparation(self.trainingData.copy(),idColumnName,itemColumnName)[2]
        self.foldSize = min(10000,self.aggTrainingData.shape[0],fold_size)
        
        
    def fit(self):
        cosine_weightingsPerFold,jaccard_weightingsPerFold,optimalScoresPerFold = kFolds.k_folds_optimization( self.trainingData, self.idColumnName,self.itemColumnName,self.kFolds, self.aggTrainingData, self.aggTestData, self.nRecommendations, gradientStep=self.gradientStepSize, foldSize=self.foldSize, fold_sample_size=min(1000,self.foldSize/2),evaluationMethod=self.evaluationMethod)
        self.optimalJaccardWeightings = np.mean(jaccard_weightingsPerFold,axis=0)
        self.optimalCosineWeightings = np.mean(cosine_weightingsPerFold,axis=0)
    ...
    def score(self):
        MRR3scoresPerFold = kFolds.k_folds( self.trainingData, self.idColumnName,self.itemColumnName,self.kFolds, self.aggTrainingData, self.aggTestData, self.nRecommendations, self.optimalCosineWeightings, self.optimalJaccardWeightings, foldSize=self.foldSize,evaluationMethod="MRR3")
        MRR3modelScore = np.mean(MRR3scoresPerFold)
        MRR4scoresPerFold = kFolds.k_folds( self.trainingData,self.idColumnName,self.itemColumnName, self.kFolds, self.aggTrainingData, self.aggTestData, self.nRecommendations, self.optimalCosineWeightings, self.optimalJaccardWeightings, foldSize=self.foldSize,evaluationMethod="MRR4")
        MRR4modelScore = np.mean(MRR4scoresPerFold)
        return {'MRR_3 score':MRR3modelScore, 'MRR_4 score': MRR4modelScore}
    
    def recommend(self):
        #!!!!!!!!!!!!!!!!!!!!!!
        #function to get weights for new columns not in training set
        #!!!!!!!!!!!!!!!!!!!!!!!!
        self.recommendationData = recommendationScore.RecommendationScores(self.nRecommendations,self.optimalCosineWeightings, self.optimalJaccardWeightings,self.itemColumnsTest,self.nCategoricalColumnsTest,self.aggTestData,self.itemColumnName )
