...
def backtesting(backDatedData,upToDateData,trainedDataIds,idColumnName,trainingDataWithRecoomendations, evaluationMethod='MRR4'):
    #a subset of backDatedData was used to train the model and recommendations generated, this data with the recommendations is trainingDataWithRecoomendations
    ...
    joinedData['new_item_flag'] = (joinedData['item_count_x'] < joinedData['item_count_c']).astype(int)
    ...
    del joinedData
    newItemData = pd.merge(newItemData,trainingDataWithRecoomendations[[idColumnName,'no.1 most recommended item','no.1 item recommendation score','no.2 most recommended item','no.2 item recommendation score','no.3 most recommended item','no.3 item recommendation score','no.4 most recommended item','no.4 item recommendation score','no.5 most recommended item','no.5 item recommendation score','no.6 most recommended item','no.6 item recommendation score']],
                      on = idColumnName, 
